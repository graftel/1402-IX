...
namespace Neo.ApplicationFramework.Generated
{
    ...
    public partial class ImportData
    {
		...
		void ImportData_Opened(System.Object sender, System.EventArgs e)
		{
		...
		void btnOK_Click(System.Object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				else
				{
					...
					try
					{
						var reader = new StreamReader(File.OpenRead(file_path[selectedIndex]));
						...
						while (!reader.EndOfStream)
						{
							...
							foreach(string strTmp in values)
							{
								if (strTmp != "")
								{
									if ((int)Globals.Tags.SelectType.Value == 0)
									{
										if (!CheckRepeat(strTmp, Globals.Tags.strFileNameList.Value.String)) 
										{
											Globals.Tags.strFileNameList.Value = Globals.Tags.strFileNameList.Value.String + "," + strTmp;
											numofInput++;
										}
									}
									else
									{
										if (!CheckRepeat(strTmp, Globals.Tags.strTesterInitialList.Value.String)) 
										{
											Globals.Tags.strTesterInitialList.Value = Globals.Tags.strTesterInitialList.Value.String + "," + strTmp;
											numofInput++;
										}
									}
					...
					if (isError)
					{
						Globals.Tags.GeneralMessage.Value = "Error reading file, please check if the format is csv.";
						Globals.GeneralMsgBox.Show();
					}
					else
					{
						Globals.Tags.GeneralMessage.Value = numofInput.ToString() + " name(s) have been imported";
						Globals.GeneralMsgBox.Show();
					}
